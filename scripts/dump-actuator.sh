...
# scripts/dump-jattach.sh (tier 2) or scripts/dump-threads.sh /
# scripts/dump-heap.sh (tier 3, ephemeral JDK container).
#
# Usage:
...
case "$ACTION" in
    threads)
        ...
        if ! kubectl -n "$NAMESPACE" exec "$POD" -c "$APP_CONTAINER" -- \
            ...
            err "  scripts/dump-jattach.sh threads -n $NAMESPACE $POD"
            rm -f "$LOCAL_PATH"
            exit 1
        fi
    ...
    heap)
        ...
        if ! kubectl -n "$NAMESPACE" exec "$POD" -c "$APP_CONTAINER" -- \
            ...
            err "  scripts/dump-jattach.sh heap --confirm -n $NAMESPACE $POD"
            rm -f "$LOCAL_PATH"
            exit 1
        fi
