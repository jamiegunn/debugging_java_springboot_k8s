...
# checks, see scripts/k3s/verify/smoke.sh.
#
# Prereqs:
#   - a healthy k3s stack (scripts/k3s/verify/doctor.sh) — valkey-cli runs in-cluster,
...
if ! "${VK_EXEC[@]}" -h "$SEED" -p "$PORT" -a "$PASS" --no-auth-warning ping 2>/dev/null | grep -q PONG; then
    ...
    err "check: scripts/k3s/verify/doctor.sh"
    exit 1
fi
