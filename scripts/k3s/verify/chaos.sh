...
scripts/k3s/verify/chaos.sh probe
...
scripts/k3s/verify/chaos.sh status                 # $K3S_LB_VM Stopped, VIP (none)
...
scripts/k3s/phases/lb.sh up        # re-assert keepalived VIP + HAProxy backends
EOF
}
...
run_break() {
    case "$1" in
        node-down)     break_node_down "${2:-}";  probe; sect "DEBUG"; debug_node_down "${2:-}" | while IFS= read -r l; do case "$l" in \#*) note "${l#\# }";; *) cmd "$l";; esac; done; sect "HEAL — run: scripts/k3s/verify/chaos.sh heal node-down"; heal_node_down "${2:-}" | while IFS= read -r l; do cmd "$l"; done ;;
        lb-down)       break_lb_down; probe; sect "DEBUG"; debug_lb_down | while IFS= read -r l; do case "$l" in \#*) note "${l#\# }";; *) cmd "$l";; esac; done; sect "HEAL — run: scripts/k3s/verify/chaos.sh heal lb-down"; heal_lb_down | while IFS= read -r l; do case "$l" in \#*) note "${l#\# }";; *) cmd "$l";; esac; done ;;
        ...
        oracle-down|mq-down|valkey-down) ns="${1%-down}"; break_backend "$ns"; probe; sect "DEBUG"; debug_backend "$ns" | while IFS= read -r l; do case "$l" in \#*) note "${l#\# }";; *) cmd "$l";; esac; done; sect "HEAL — run: scripts/k3s/verify/chaos.sh heal $1"; heal_backend "$ns" | while IFS= read -r l; do cmd "$l"; done ;;
...
run_heal() {
    case "$1" in
        ...
        lb-down)       limactl start "$K3S_LB_VM" >/dev/null 2>&1; "$SCRIPTS_ROOT/k3s/phases/lb.sh" up >/dev/null 2>&1; echo "  LB VM started + VIP/HAProxy re-asserted" ;;
