...
else bad "VIP $K3S_VIP not reachable from the Mac" "scripts/k3s/phases/lb.sh status"; fi
...
else bad "HTTP by hostname to $APP_HOST failed" "scripts/k3s/verify/doctor.sh"; fi
...
else bad "$K3S_LB_VM does not hold the VIP" "scripts/k3s/phases/lb.sh up"; fi
...
else bad "MetalLB controller not Ready" "scripts/k3s/phases/platform.sh up"; fi
...
else bad "Valkey SET/GET via hostname failed" "scripts/k3s.sh doctor; scripts/k3s/tours/valkey-tour.sh --section topology"; fi
