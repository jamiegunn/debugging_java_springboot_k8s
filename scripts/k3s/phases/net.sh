...
cmd_up() {
    # DNS only. The VIP itself lives on the LB tier (scripts/k3s/phases/lb.sh); DNS just
    ...
    configure_mac_resolver
    echo
    info "DNS up (*.$BASE_DOMAIN → $K3S_VIP). The VIP is served by the LB tier: scripts/k3s/phases/lb.sh up"
}
...
cmd_status() {
    info "VIP $K3S_VIP (served by the LB tier — scripts/k3s/phases/lb.sh status):"
